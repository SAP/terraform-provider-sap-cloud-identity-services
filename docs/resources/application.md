...
- `provided_apis` (Attributes List) APIs provided by this application that can be consumed by other applications or referenced in public_client_apis. (see [below for nested schema](#nestedatt--authentication_schema--provided_apis))
...
- `public_client_apis` (Set of String) List of API names (from provided_apis) that are accessible to public clients. Only valid when allow_public_client_flows is true and all_apis_access is false.
...
- `index` (Number) Provide a unique index for the endpoint.
